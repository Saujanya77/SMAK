  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900 dark:to-indigo-900">
      ...
      <header className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-700/50 sticky top-0 z-40">
        <div className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 gap-2 md:gap-4 flex-wrap">
          <div className="flex items-center space-x-2 md:space-x-4 flex-wrap">
            ...
            <h1 className="text-lg md:text-xl font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Blogs</h1>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6 md:mb-8 flex-col sm:flex-row gap-4 sm:gap-0">
            <div className="flex-1">
              <h2 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-1 md:mb-2">Medical Blogs</h2>
              <p className="text-sm md:text-base text-gray-600 dark:text-gray-400">Latest stories and articles from the community</p>
            </div>
            <Button
          ...
          {featuredBlogs.length > 0 && (
            <div className="mb-6 md:mb-8">
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-3 md:mb-4">Featured Blogs</h2>
          ...
          {/* Category Filter */}
          <div className="mb-6 md:mb-8">
            <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-3 md:mb-4">Categories</h2>
              ...
              filteredBlogs.map(blog => (
                <div key={blog.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-3 md:p-4">
                    ...
                    <div className="flex items-center space-x-2 text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-2 md:mb-4">
                      <span>{blog.readTime}</span>
                      <span>•</span>
                      <span>{blog.comments} comments</span>
                    </div>
                    <p className="text-xs md:text-sm text-gray-700 dark:text-gray-300 mb-3 md:mb-4 line-clamp-2 md:line-clamp-3">
                      {blog.excerpt}
                    </p>
                    <div className="flex justify-between items-center flex-col sm:flex-row gap-3 sm:gap-0">
                      <Button
                        ...
                        className="text-blue-600 border-blue-600 hover:bg-blue-50 text-xs md:text-sm px-3 w-full sm:w-auto"
                      >
                        Read More
                      </Button>
                      <div className="flex items-center space-x-1 md:space-x-2 w-full sm:w-auto flex-wrap justify-end gap-1">
                        <Button
                          ...
                          className={`text-xs md:text-sm px-2 md:px-3 ${likedBlogs.has(blog.id) ? 'text-red-600 border-red-300 bg-red-50' : 'text-red-600 border-red-300 hover:bg-red-50'}`}
                        >
                          <Heart className={`h-3 w-3 md:h-4 md:w-4 mr-1 ${likedBlogs.has(blog.id) ? 'fill-current' : ''}`} />
                          <span className="hidden sm:inline">({blog.likes})</span>
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                        ...
                        <Button
                          ...
                          className={`text-xs md:text-sm px-2 md:px-3 ${bookmarkedBlogs.has(blog.id) ? 'bg-blue-50 border-blue-300 text-blue-700' : ''}`}
                        >
                          <Bookmark className={`h-3 w-3 md:h-4 md:w-4 mr-1 ${bookmarkedBlogs.has(blog.id) ? 'fill-current' : ''}`} />
                          <span className="hidden sm:inline">{bookmarkedBlogs.has(blog.id) ? 'Saved' : 'Save'}</span>
                        </Button>
                        ...
                        {ADMIN_EMAILS.includes(user?.email) && (
                          <Button
                            ...
                            className="text-xs md:text-sm px-2 md:px-3 border-red-300 hover:bg-red-50"
                          >
        ...
        <DialogContent className="max-w-2xl md:max-w-6xl max-h-[90vh] p-0 w-[95vw]">
          {selectedBlog && (
            <div className="h-[90vh] flex flex-col bg-gray-50 dark:bg-gray-900">
              {/* Blog Header */}
              <div className="flex items-center justify-between p-2 md:p-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 gap-2 flex-wrap">
                <div className="flex items-center space-x-2 md:space-x-4 min-w-0 flex-1">
                  <Button variant="ghost" size="sm" onClick={closePreview} className="p-1">
                    <X className="h-4 w-4" />
                  </Button>
                  <div className="min-w-0 flex-1">
                    <h3 className="font-semibold text-gray-900 dark:text-white line-clamp-1 text-xs md:text-base">
                      {selectedBlog.title}
                    </h3>
                    <div className="flex items-center space-x-1 md:space-x-2 mt-1 flex-wrap gap-1 text-xs">
                ...
                <div className="flex items-center space-x-1 md:space-x-2 flex-wrap gap-1">
                  <Button
                    ...
                    className={`text-xs md:text-sm px-2 md:px-3 ${likedBlogs.has(selectedBlog.id) ? 'text-red-600 border-red-300 bg-red-50' : 'text-red-600 border-red-300 hover:bg-red-50'}`}
                  >
                    <Heart className={`h-3 w-3 md:h-4 md:w-4 mr-1 ${likedBlogs.has(selectedBlog.id) ? 'fill-current' : ''}`} />
                    <span className="hidden sm:inline">({selectedBlog.likes})</span>
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  <Button
                    ...
                    className={`text-xs md:text-sm px-2 md:px-3 ${bookmarkedBlogs.has(selectedBlog.id) ? 'bg-blue-50 border-blue-300 text-blue-700' : ''}`}
                  >
                    <Bookmark className={`h-3 w-3 md:h-4 md:w-4 mr-1 ${bookmarkedBlogs.has(selectedBlog.id) ? 'fill-current' : ''}`} />
                    <span className="hidden sm:inline">{bookmarkedBlogs.has(selectedBlog.id) ? 'Saved' : 'Save'}</span>
                  </Button>
                </div>
              </div>
              {/* Blog Content */}
              <div className="flex-1 overflow-auto bg-white dark:bg-gray-800 p-3 md:p-6">
                <div className="max-w-2xl md:max-w-4xl mx-auto">
                  <img
                    src={selectedBlog.image}
                    alt={selectedBlog.title}
                    className="w-full h-32 sm:h-48 md:h-64 object-cover rounded-lg md:rounded-xl shadow-md md:shadow-lg mb-4 md:mb-8"
                  />
                  <div className="prose prose-sm md:prose-lg max-w-none dark:prose-invert">
                    ...
                    <div className="flex items-center space-x-2 md:space-x-4 text-xs md:text-sm text-gray-600 dark:text-gray-400 mb-4 md:mb-6 pb-4 md:pb-6 border-b border-gray-200 dark:border-gray-700 flex-wrap gap-2">
                      <div className="flex items-center space-x-2">
                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-800 font-bold text-xs md:text-sm">
                          {user?.name ? user.name.charAt(0).toUpperCase() : "U"}
                        </div>
                        <div>
                          <p className="font-medium text-gray-900 dark:text-white text-xs md:text-sm">{selectedBlog.author}</p>
                          <p className="text-xs text-gray-500">{selectedBlog.publishedDate}</p>
                        </div>
                      </div>
                      <span className="hidden sm:inline">•</span>
                      <span className="text-xs md:text-sm">{selectedBlog.readTime}</span>
                      <span className="hidden sm:inline">•</span>
                      <span className="flex items-center text-xs md:text-sm hidden sm:flex">
                    ...
                    <div
                      className="text-xs md:text-base text-gray-700 dark:text-gray-300 leading-relaxed blog-content"
                      dangerouslySetInnerHTML={{ __html: selectedBlog.content }}
                      style={{ fontSize: '14px', lineHeight: '1.8' }}
                    />
                    <div className="mt-8 md:mt-12 pt-4 md:pt-6 border-t border-gray-200 dark:border-gray-700">
                      <div className="flex items-center justify-between flex-col md:flex-row gap-4">
                        <div className="flex items-center space-x-3 md:space-x-6">
                          <Button
                            variant="outline"
                            onClick={() => handleLikeBlog(selectedBlog.id)}
                            className={`flex items-center space-x-1 md:space-x-2 text-xs md:text-sm ${likedBlogs.has(selectedBlog.id) ? 'text-red-600 border-red-300 bg-red-50' : 'text-red-600 border-red-300 hover:bg-red-50'}`}
                          >
                            <Heart className={`h-3 w-3 md:h-4 md:w-4 ${likedBlogs.has(selectedBlog.id) ? 'fill-current' : ''}`} />
                        ...
                        <div className="flex items-center space-x-1 md:space-x-2 flex-wrap gap-1 md:gap-0">
                          ...
                          >
                            <Bookmark className={`h-3 w-3 md:h-4 md:w-4 mr-1 ${bookmarkedBlogs.has(selectedBlog.id) ? 'fill-current' : ''}`} />
                            <span className="hidden sm:inline">{bookmarkedBlogs.has(selectedBlog.id) ? 'Saved' : 'Save for Later'}</span>
                            <span className="sm:hidden">{bookmarkedBlogs.has(selectedBlog.id) ? 'Saved' : 'Save'}</span>
                          </Button>
                        </div>
                      </div>
                    </div>
      ...
      <Dialog open={showComments} onOpenChange={closeComments}>
        <DialogContent className="max-w-xl md:max-w-2xl max-h-[80vh] w-[95vw]">
          <DialogHeader>
          ...
          <div className="flex flex-col h-[60vh]">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 md:space-y-4 mb-3 md:mb-4">
              {commentBlogId && comments[commentBlogId] && comments[commentBlogId].length > 0 ? (
                comments[commentBlogId].map((comment) => (
                  <div key={comment.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 md:p-4">
                    <div className="flex items-center space-x-2 mb-2">
                      <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-800 font-bold text-xs md:text-sm">
                        {comment.author ? comment.author.charAt(0).toUpperCase() : "U"}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-xs md:text-sm text-gray-900 dark:text-white">{comment.author}</p>
                        <p className="text-xs text-gray-500">{comment.date}</p>
                      </div>
                    </div>
                    <p className="text-xs md:text-sm text-gray-700 dark:text-gray-300">{comment.text}</p>
                  </div>
                ))
              ) : (
                <div className="text-center text-gray-500 py-6 md:py-8">
                  <MessageCircle className="h-10 w-10 md:h-12 md:w-12 mx-auto mb-3 md:mb-4 opacity-50" />
                  <p className="text-xs md:text-sm">No comments yet. Be the first!</p>
                </div>
              )}
            </div>
            {/* Add Comment Form */}
            <div className="border-t pt-3 md:pt-4">
              <div className="flex space-x-2 md:space-x-3">
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-800 font-bold flex-shrink-0 text-xs md:text-sm">
                  {user?.name ? user.name.charAt(0).toUpperCase() : "U"}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex justify-end mt-2">
                    <Button
                      ...
                      size="sm"
                      className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-xs md:text-sm px-3"
                    >
