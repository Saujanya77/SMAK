...
const Members = () => {
    ...
    ? membersData
      ...
      .map((item) => {
        ...
        if (item.image.includes("drive.google.com")) {
          const idMatch = item.image.match(/id=([\w-]+)/) || item.image.match(/\/d\/([\w-]+)/);
          const id = idMatch ? idMatch[1] : "";
          pictureUrl = id ? `https://drive.google.com/uc?export=view&id=${id}` : item.image;
        } else {
  ...
  // Filter members from Firestore
  const filteredMainMembers = firestoreMembers.filter((member) =>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-blue-700 dark:text-blue-300 mb-4">All Members</h2>
